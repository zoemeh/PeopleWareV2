<div class="grid {{ $formVisible ? 'grid-cols-2' : '' }} gap-4">
                    ...
                    <div class="p-1.5 min-w-full inline-block align-middle">
                        <div class="overflow-hidden">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead>
                                    <tr>
                                        @foreach (['ID', 'Nombre', 'Email'] as $columna)
                                            <th scope="col"
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="btn-group" role="group">
                                                    <button type="button"
                                                        ...
                                                        wire:click="show({{ $usuario->id }})">Ver</button>
                                                    <button type="button"
                                                        class="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800"
                                                        wire:click="update({{ $usuario->id }})">Editar</button>
                                                    <button
                                                        ...
                                                        wire:click="$emit('confirmDelete', {{ $usuario->id }}, '{{ json_encode($usuario::class) }}', '{{ $usuario->nombre }}', )"
    ...
    @if ($formVisible)
        <livewire:usuario-form :usuario="$currentUsuario" />
    @endif
</div>
