        <form method="POST"
            action="{{ $idioma->id ? route('idiomas.update', $idioma->id) : route('idiomas.store') }}">
            @if ($idioma->id)
                @method('PUT')
            @endif
            @csrf
            <label for="descripcion" class="block text-sm font-medium mb-2 dark:text-white">Descripcion</label>
            <input type="text" id="descripcion" name="descripcion" value="{{ $idioma->descripcion }}"
            ...
            <div class="flex mb-2">
                <input type="checkbox" id="activo" name="activo" @checked($idioma->activo)
