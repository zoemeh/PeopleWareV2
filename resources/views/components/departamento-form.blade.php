        <form method="POST"
            action="{{ $departamento->id ? route('departamentos.update', $departamento->id) : route('departamentos.store') }}">
            @if ($departamento->id)
                @method('PUT')
            @endif
            @csrf
            ...
            <div class="flex mb-2">
                <input type="checkbox" id="activo" name="activo" @checked($departamento->activo)
